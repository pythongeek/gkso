 ...
 *   Phase 1 – Site-wide content indexing (TF-IDF, entities, keyword meta)
 *   Phase 2 – Anchor text candidate generation (3 methods)
 *   Phase 3 – Candidate filtering pipeline
 *   Phase 4 – 5-signal URL-to-anchor scoring  (35/30/15/10/10)
 *   Phase 5 – Placement & position logic
 *   Phase 6 – User instruction override layer → n8n → store suggestions
 *
 * Database tables:
 *   wp_gkso_link_index       – per-post semantic index entries
 *   wp_gkso_link_suggestions – AI-generated link proposals (approve/reject)
 */
...
class GKSO_Internal_Linking {
    ...
    public static function init() {
        ...
        gkso_log( 'GKSO_Internal_Linking initialised', null, 'INFO' );
    }
    ...
    public static function index_post( $post_id ) {
        ...
        gkso_log( "Indexing post #{$post_id}: {$post->post_title}", null, 'DEBUG' );
        ...
        $wpdb->replace( $wpdb->prefix . self::TABLE_INDEX, [
            'post_id'       => $post_id,
    ...
    public static function remove_from_index( $post_id ) {
        ...
        gkso_log( "Post #{$post_id} removed from link index", null, 'INFO' );
    }

    public static function full_reindex() {
        gkso_log( 'Starting full site re-index', null, 'INFO' );
        $enabled = get_option( 'gkso_enabled_post_types', [ 'post', 'page' ] );
        $posts   = get_posts( [ 'post_type' => $enabled, 'post_status' => 'publish', 'posts_per_page' => -1, 'fields' => 'ids' ] );
    ...
    public static function reindex_inbound_counts() {
        ...
        gkso_log( 'Inbound link counts refreshed for ' . count( $posts ) . ' posts', null, 'INFO' );
    }

    // ── Phase 2 & 3 – Candidate generation and filtering ─────────────────────────
    ...
    public static function generate_candidates( $source_post_id, $options = [] ) {
        ...
        // Method A + C combined (noun phrase extraction seeded by top TF-IDF terms)
        $noun_phrases        = self::extract_noun_phrases( $plain_text );
        // Method B: keyword reverse lookup
        $kw_candidates       = self::keyword_reverse_lookup( $plain_text, $source_post_id );
        // Method C: semantic candidates
        ...
        foreach ( $all as $candidate ) {
            ...
            if ( preg_match( '/<h[1-4][^>]*>[^<]*' . preg_quote( $anchor, '/' ) . '[^<]*<\/h[1-4]>/i', $content ) ) { continue; }
            if ( isset( $candidate['target_url'] ) && $candidate['target_url'] === get_permalink( $source_post_id ) ) { continue; }
            ...
            foreach ( $link_positions_used as $used ) {
                if ( abs( $word_offset - $used ) < 50 ) { $too_close = true; break; }
            }
            if ( $too_close ) { continue; }
            if ( count( $link_positions_used ) >= $max_links ) { continue; }

            // Avoid first paragraph if configured
    ...
    public static function score_candidates( $source_post_id, $candidates, $weights = [] ) {
        ...
        foreach ( $candidates as $candidate ) {
            ...
            foreach ( $targets as $target ) {
                ...
                if ( $inbound === 0 )                            { $orphan = 1.0; }
                elseif ( $inbound === 1 )                        { $orphan = 0.8; }
                elseif ( $inbound >= self::ORPHAN_INBOUND_MAX )  { $orphan = 0.1; }
                else                                             { $orphan = 1.0 - ( $inbound / self::ORPHAN_INBOUND_MAX ) * 0.9; }
                ...
                $composite = $semantic * $w['semantic'] + $keyword * $w['keyword'] + $authority * $w['authority'] + $orphan * $w['orphan'] + $recency * $w['recency'];
            ...
            if ( ( $best_signals['semantic'] ?? 0 ) > 0.85 && ( $best_signals['keyword'] ?? 0 ) > 0.70 ) { $method = 'ensemble'; }
    ...
    public static function run_analysis( $post_id, $options = [] ) {
        ...
        gkso_log( count( $candidates ) . " candidates generated", null, 'DEBUG' );

        $scored = self::score_candidates( $post_id, $candidates, self::get_algorithm_weights() );
        gkso_log( count( $scored ) . " scored suggestions", null, 'DEBUG' );

        if ( ! empty( get_option( 'gkso_n8n_webhook_url' ) ) ) {
    ...
    private static function send_to_n8n_for_validation( $post_id, $scored, $options ) {
        ...
        $response  = wp_remote_post( get_option( 'gkso_n8n_webhook_url' ), [
            'timeout' => 30,
            'headers' => [ 'Content-Type' => 'application/json', 'X-GKSO-Signature' => $signature ],
            'body'    => wp_json_encode( $payload ),
        ] );
        if ( is_wp_error( $response ) ) {
            gkso_log( 'n8n link webhook failed: ' . $response->get_error_message(), null, 'WARNING' );
            return self::store_suggestions( $post_id, $scored );
        }
        gkso_log( "Link analysis payload sent to n8n for post #{$post_id}", null, 'INFO' );
        return $scored;
    }
    ...
    public static function store_suggestions( $post_id, $scored, $n8n_run_id = '' ) {
        ...
        foreach ( $scored as $s ) {
            ...
            $wpdb->insert( $table, [
                ...
                'created_at'      => $now,
                'updated_at'      => $now,
            ], [ '%d','%d','%s','%s','%f','%f','%f','%f','%f','%f','%s','%s','%d','%s','%s','%s','%s','%s' ] );
            $ids[] = $wpdb->insert_id;
        }
    ...
    public static function apply_suggestion( $suggestion_id ) {
        ...
        if ( ! $row ) { return new WP_Error( 'not_found', __( 'Suggestion not found.', 'gemini-kimi-seo' ) ); }
        if ( $row['status'] !== 'approved' ) { return new WP_Error( 'not_approved', __( 'Must be approved first.', 'gemini-kimi-seo' ) ); }

        $post = get_post( (int) $row['source_post_id'] );
        if ( ! $post ) { return new WP_Error( 'post_not_found', __( 'Post not found.', 'gemini-kimi-seo' ) ); }
        ...
        if ( ! $count ) { return new WP_Error( 'not_replaced', __( 'Anchor text not found in content.', 'gemini-kimi-seo' ) ); }

        wp_update_post( [ 'ID' => $post->ID, 'post_content' => $new_content ] );

        $wpdb->update( $table, [ 'status' => 'applied', 'updated_at' => current_time( 'mysql' ) ], [ 'id' => $suggestion_id ], [ '%s','%s' ], [ '%d' ] );

        // Update outbound map & inbound count
        $outbound   = json_decode( $wpdb->get_var( $wpdb->prepare( "SELECT outbound_urls FROM {$wpdb->prefix}" . self::TABLE_INDEX . " WHERE post_id = %d", $post->ID ) ), true ) ?: [];
        $outbound[] = $row['target_url'];
        $wpdb->update( $wpdb->prefix . self::TABLE_INDEX, [ 'outbound_urls' => wp_json_encode( array_unique( $outbound ) ) ], [ 'post_id' => $post->ID ], [ '%s' ], [ '%d' ] );
        $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}" . self::TABLE_INDEX . " SET inbound_count = inbound_count + 1 WHERE post_id = %d", (int) $row['target_post_id'] ) );

        gkso_log( "Suggestion #{$suggestion_id} applied to post #{$post->ID}", null, 'INFO' );
        return true;
    }
    ...
    public static function register_rest_routes() {
        $ns = GKSO_REST_NAMESPACE;

        register_rest_route( $ns, '/run-link-analysis', [
            'methods' => WP_REST_Server::CREATABLE, 'callback' => [ __CLASS__, 'rest_run_analysis' ],
            'permission_callback' => fn() => current_user_can( 'seo_optimize' ),
            'args' => [ 'post_id' => [ 'required' => true, 'type' => 'integer', 'sanitize_callback' => 'absint' ] ],
        ] );
        register_rest_route( $ns, '/link-suggestions/(?P<post_id>\d+)', [
            'methods' => WP_REST_Server::READABLE, 'callback' => [ __CLASS__, 'rest_get_suggestions' ],
            'permission_callback' => fn() => current_user_can( 'seo_view_tests' ),
        ] );
        register_rest_route( $ns, '/link-suggestions', [
            'methods' => WP_REST_Server::READABLE, 'callback' => [ __CLASS__, 'rest_get_all_suggestions' ],
            'permission_callback' => fn() => current_user_can( 'seo_view_tests' ),
        ] );
        register_rest_route( $ns, '/link-suggestions/(?P<id>\d+)/status', [
            'methods' => WP_REST_Server::EDITABLE, 'callback' => [ __CLASS__, 'rest_update_status' ],
            'permission_callback' => fn() => current_user_can( 'seo_optimize' ),
            'args' => [ 'status' => [ 'required' => true, 'type' => 'string', 'enum' => [ 'approved','rejected','pending' ] ] ],
        ] );
        register_rest_route( $ns, '/link-suggestions/(?P<id>\d+)/apply', [
            'methods' => WP_REST_Server::CREATABLE, 'callback' => [ __CLASS__, 'rest_apply_suggestion' ],
            'permission_callback' => fn() => current_user_can( 'seo_optimize' ),
        ] );
        register_rest_route( $ns, '/link-suggestions/bulk-status', [
            'methods' => WP_REST_Server::EDITABLE, 'callback' => [ __CLASS__, 'rest_bulk_status' ],
            'permission_callback' => fn() => current_user_can( 'seo_optimize' ),
            'args' => [ 'ids' => [ 'required' => true, 'type' => 'array' ], 'status' => [ 'required' => true, 'type' => 'string' ] ],
        ] );
        register_rest_route( $ns, '/link-index/health', [
            'methods' => WP_REST_Server::READABLE, 'callback' => [ __CLASS__, 'rest_index_health' ],
            'permission_callback' => fn() => current_user_can( 'seo_view_tests' ),
        ] );
        register_rest_route( $ns, '/link-index/reindex', [
            'methods' => WP_REST_Server::CREATABLE, 'callback' => [ __CLASS__, 'rest_reindex' ],
        ...
        register_rest_route( $ns, '/update-link-suggestions', [
            'methods' => WP_REST_Server::CREATABLE, 'callback' => [ __CLASS__, 'rest_n8n_callback' ],
            'permission_callback' => [ __CLASS__, 'verify_n8n_permission' ],
        ] );
        register_rest_route( $ns, '/link-pillars', [
            'methods' => WP_REST_Server::READABLE . ',' . WP_REST_Server::CREATABLE,
            'callback' => [ __CLASS__, 'rest_pillars' ],
            'permission_callback' => fn() => current_user_can( 'seo_view_tests' ),
        ] );
    }
    ...
    public static function rest_run_analysis( WP_REST_Request $req ) {
        ...
        if ( ! get_post( $pid ) ) { return new WP_Error( 'not_found', 'Post not found.', [ 'status' => 404 ] ); }
    ...
    public static function rest_get_suggestions( WP_REST_Request $req ) {
        ...
        $rows = $wpdb->get_results( $wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}" . self::TABLE_SUGGESTIONS . " WHERE source_post_id = %d ORDER BY confidence DESC", $pid
        ), ARRAY_A );
    ...
    public static function rest_bulk_status( WP_REST_Request $req ) {
        ...
        if ( empty( $ids ) ) { return new WP_Error( 'empty', 'No IDs.', [ 'status' => 400 ] ); }
    ...
    public static function rest_index_health( WP_REST_Request $req ) {
        global $wpdb;
        $it = $wpdb->prefix . self::TABLE_INDEX;
        $st = $wpdb->prefix . self::TABLE_SUGGESTIONS;

        $pillars_data = [];
        foreach ( get_option( 'gkso_pillar_pages', [] ) as $pid ) {
            $entry = $wpdb->get_row( $wpdb->prepare( "SELECT inbound_count FROM {$it} WHERE post_id = %d", $pid ), ARRAY_A );
            if ( $entry ) {
                $pillars_data[] = [ 'id' => $pid, 'title' => get_the_title( $pid ), 'url' => get_permalink( $pid ), 'inboundLinks' => (int) $entry['inbound_count'], 'priority' => 'high' ];
            }
        }

        return rest_ensure_response( [
            'postsIndexed'       => (int)   $wpdb->get_var( "SELECT COUNT(*) FROM {$it}" ),
            'totalPosts'         => (int)   $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_status='publish' AND post_type IN ('post','page')" ),
            'linksApplied'       => (int)   $wpdb->get_var( "SELECT COUNT(*) FROM {$st} WHERE status='applied'" ),
            'orphanCount'        => (int)   $wpdb->get_var( "SELECT COUNT(*) FROM {$it} WHERE inbound_count <= 1" ),
            'avgLinksPerPost'    => round( (float) $wpdb->get_var( "SELECT AVG(inbound_count) FROM {$it}" ), 1 ),
            'pendingSuggestions' => (int)   $wpdb->get_var( "SELECT COUNT(*) FROM {$st} WHERE status='pending'" ),
            'pillarPages'        => $pillars_data,
        ] );
    }

    public static function rest_pillars( WP_REST_Request $req ) {
        if ( in_array( $req->get_method(), [ 'POST', 'PUT' ], true ) ) {
            $pid     = absint( $req->get_param( 'post_id' ) );
            $pillars = get_option( 'gkso_pillar_pages', [] );
            $pillars[] = $pid;
            update_option( 'gkso_pillar_pages', array_unique( $pillars ) );
            return rest_ensure_response( [ 'success' => true ] );
        }
        return self::rest_index_health( $req );
    }
    ...
    public static function rest_n8n_callback( WP_REST_Request $req ) {
        ...
        if ( ! $pid || empty( $list ) ) { return new WP_Error( 'invalid', 'Missing data.', [ 'status' => 400 ] ); }
    ...
    public static function rest_link_config( WP_REST_Request $req ) {
        if ( in_array( $req->get_method(), [ 'POST','PUT','PATCH' ], true ) ) {
            $allowed = [ 'gkso_link_min_confidence','gkso_link_max_per_post','gkso_link_min_word_distance','gkso_link_weight_semantic','gkso_link_weight_keyword','gkso_link_weight_authority','gkso_link_weight_orphan','gkso_link_weight_recency','gkso_link_auto_approve','gkso_link_auto_approve_threshold','gkso_link_avoid_headings','gkso_link_avoid_first_para','gkso_link_avoid_blockquotes','gkso_link_prefer_early','gkso_link_one_url_per_post','gkso_link_instructions' ];
            foreach ( $allowed as $key ) {
                $val = $req->get_param( str_replace( 'gkso_link_', '', $key ) ) ?? $req->get_json_params()[ $key ] ?? null;
                if ( null !== $val ) { update_option( $key, $val ); }
            }
            // Also try direct JSON body keys
            $body = $req->get_json_params() ?? [];
            foreach ( $allowed as $key ) {
                if ( isset( $body[ $key ] ) ) { update_option( $key, $body[ $key ] ); }
            }
            return rest_ensure_response( [ 'success' => true ] );
        }
    ...
    public static function rest_reindex( WP_REST_Request $req ) {
        return rest_ensure_response( [ 'success' => true, 'queued' => self::full_reindex() ] );
    }

    public static function verify_n8n_permission( WP_REST_Request $req ) {
        $ip  = GKSO_Security::check_ip_allowlist();
        if ( is_wp_error( $ip ) ) { return $ip; }
        $sig = GKSO_Security::verify_n8n_signature( $req );
        return ! is_wp_error( $sig );
    }
    ...
    private static function compute_tfidf( $text ) {
        ...
        $stop  = [ 'the','a','an','and','or','but','in','on','at','to','for','of','with','is','are','was','were','be','been','it','its','this','that','these','those','by','as','from','up','out','if','then','so','not' ];
        $tf    = [];
        foreach ( $words as $w ) {
        ...
        if ( empty( $tf ) ) { return []; }
        $total = array_sum( $tf );
        foreach ( $tf as $k => $v ) { $tf[ $k ] = round( $v / $total, 6 ); }
        arsort( $tf );
    ...
    private static function cosine_similarity( $a, $b ) {
        ...
        $dot = $ma = $mb = 0.0;
        foreach ( $a as $k => $v ) { $dot += $v * ( $b[ $k ] ?? 0 ); $ma += $v * $v; }
    ...
    private static function anchor_to_tfidf( $phrase ) {
        ...
        $tf    = [];
        foreach ( $words as $w ) { $tf[ $w ] = ( $tf[ $w ] ?? 0 ) + 1; }
        $total = array_sum( $tf );
        foreach ( $tf as $k => $v ) { $tf[ $k ] = $v / $total; }
        return $tf;
    }
    ...
    private static function keyword_alignment_score( $anchor, array $kws ) {
        ...
        foreach ( $kws as $kw ) {
            $kl = strtolower( $kw );
            if ( $al === $kl ) { return 1.0; }
            if ( strpos( $al, $kl ) !== false || strpos( $kl, $al ) !== false ) { $best = max( $best, 0.75 ); continue; }
            $aw = explode( ' ', $al ); $kw = explode( ' ', $kl );
            $overlap = count( array_intersect( $aw, $kw ) );
            if ( $overlap ) { $best = max( $best, 0.3 + ( $overlap / max( count( $aw ), count( $kw ) ) ) * 0.45 ); }
        }
        return round( $best, 4 );
    }
        ...
        foreach ( $sentences as $sent ) {
            ...
            for ( $len = 2; $len <= 5; $len++ ) {
                for ( $i = 0; $i <= $n - $len; $i++ ) {
                    ...
                    $context = '...' . substr( $sent, max( 0, $pos - 60 ), 140 ) . '...';
        ...
        foreach ( $entries as $entry ) {
            ...
            foreach ( $kws as $kw ) {
                ...
                if ( strpos( $tl, strtolower( $kw ) ) !== false ) {
                    $pos     = strpos( $tl, strtolower( $kw ) );
                    $context = '...' . substr( $text, max( 0, $pos - 60 ), 140 ) . '...';
                    $candidates[] = [ 'phrase' => $kw, 'method' => 'keyword', 'target_post_id' => (int) $entry['post_id'], 'target_url' => $entry['site_url'], 'context' => $context ];
                    break;
                }
            }
        }
    ...
    private static function semantic_phrase_candidates( $text, $source_post_id ) {
        ...
        foreach ( $phrases as $p ) {
            ...
            foreach ( $top as $term ) {
                if ( strpos( $pl, $term ) !== false ) { $candidates[] = array_merge( $p, [ 'method' => 'semantic' ] ); break; }
            }
        }
        return $candidates;
    }
    ...
    private static function get_focus_keywords( $post_id ) {
        ...
        if ( $rm ) { foreach ( explode( ',', $rm ) as $k ) { $kws[] = trim( $k ); } }
        $aio = get_post_meta( $post_id, '_aioseo_keywords', true );
        if ( $aio ) {
            $d = json_decode( $aio, true );
            if ( is_array( $d ) ) { foreach ( $d as $k ) { $kws[] = is_array( $k ) ? ( $k['label'] ?? '' ) : $k; } }
        }
    ...
    private static function extract_entities( $post ) {
        ...
        foreach ( preg_split( '/\s+/', $post->post_title ) as $w ) {
            if ( strlen( $w ) > 3 && ctype_upper( $w[0] ) ) { $entities[] = $w; }
        }
        preg_match_all( '/<h2[^>]*>([^<]+)<\/h2>/i', $post->post_content, $h2s );
        foreach ( $h2s[1] as $h ) { $entities[] = wp_strip_all_tags( $h ); }
...
add_action( 'gkso_index_single_post', [ 'GKSO_Internal_Linking', 'index_post' ] );
