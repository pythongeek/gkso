...
// Files are named: class-gkso-{slug}.php  (e.g. class-gkso-activator.php)
// Class names are: GKSO_{Slug}            (e.g. GKSO_Activator)
// Mapping: GKSO_Activator → class-gkso-activator.php
...
spl_autoload_register( function( $class ) {
    ...
    // Convert GKSO_Rest_Controller → class-gkso-rest-controller.php
    // Keep the full class name, just lowercase and replace _ with -
...
// ─── Boot on init (WP 6.7+ compatible) ─────────────────────────────────────────
add_action( 'init', function() {
    ...
    gkso_log( 'Plugin booting on init (WordPress ' . get_bloginfo( 'version' ) . ', PHP ' . PHP_VERSION . ')', null, 'INFO' );
    ...
    try {
        ...
        if ( class_exists( 'GKSO_Debug_Log' ) ) {
            GKSO_Debug_Log::init();
        }
        gkso_log( 'All modules initialized', null, 'INFO' );
    } catch ( Throwable $e ) {
...
}, 0 ); // Priority 0 to load early but after init has started